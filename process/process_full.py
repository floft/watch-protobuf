...
from writers import TFRecordWriterFullData
...
def process_watch(watch_number):
    ...
    labeled_writer = TFRecordWriterFullData(watch_number,
        postfix="labeled")
    unlabeled_writer = TFRecordWriterFullData(watch_number,
        postfix="unlabeled")

    # Keep track of how many of each category/type we found
    location_categories = collections.defaultdict(int)

    # Save labeled/unlabeled data separately
    unlabeled_x_dms = []
    unlabeled_x_accs = []
    unlabeled_x_locs = []
    unlabeled_resps = []
